...
// --bnd-* layer). Each export BINDS the framework to the existing CSS variables, so
...
// --bnd-foo-bar  →  { name:"foo-bar", var:"--bnd-foo-bar", value, camel:"fooBar" }
const tokens = [];
for (const m of css.matchAll(/(--bnd-[a-z0-9-]+):\s*([^;]+);/g)) {
  const cssVar = m[1], value = m[2].trim();
  const name = cssVar.replace(/^--bnd-/, "");
...
// 2. StyleX — defineVars whose values reference the existing --bnd-* vars
const stylex =
...
// 3. Vanilla Extract — a global theme CONTRACT mapped to the existing --bnd-* names
const ve =
